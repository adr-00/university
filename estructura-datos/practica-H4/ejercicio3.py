...
class LinkedBinaryTree:
    ...
    def add_right(self, node, e):# crea hijo derecha de node y almacena e, devuelve su nodo
        if node._right is not None: 
    ...
    def breadthfirst(self):
        if not self.is_empty():
            ...
            while not q.is_empty():
                ...
                for c in self.children(node):
                    q.enqueue(c)
    def _escribir_arbol(self, node, d):
        cadena = 2*d*' '+str(node.element()) + '\n'
        if not self.is_leaf(node):
            if self.left(node) is not None:
                cadena += self._escribir_arbol(self.left(node), d+1)
            else:
                cadena += 2*(d+1)*' '+'None' + '\n'
            if self.right(node):
                cadena += self._escribir_arbol(self.right(node), d+1)
            else:
                cadena += 2*(d+1)*' '+'None' + '\n'
        return cadena
    
    def __str__(self):
        return self._escribir_arbol(self.root(), 0)